...
namespace slimstd {
template <typename T, bool Is_Array = false> class shared_ptr {
public:
  shared_ptr() {}
  ...
  explicit operator bool() const noexcept { return this->ref_count[0] > 0; }
...
private:
  inline void inc_ref() { this->ref_count[0] += 1; }
...
// make_shared<int[]>(10)
template <class T>
enable_if_t<is_unbounded_array_v<T>, shared_ptr<remove_all_extents_t<T>, true>>
